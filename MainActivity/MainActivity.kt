package com.example.pulsemeasure
...
import com.example.pulsemeasure.databinding.ActivityMainBinding
import java.io.File
...
class MainActivity : AppCompatActivity(), SurfaceHolder.Callback {
        ...
        if (camera != null) {
            try {
                ...
                if (flashEnabled) {
                    ...
                    binding.flashButton.text = "Включить вспышку"
                } else {
                    parameters?.flashMode = Camera.Parameters.FLASH_MODE_TORCH
                    binding.flashButton.text = "Выключить вспышку"
                }
    ...
    private fun startHeartRateReading() {
        if (processing.compareAndSet(false, true)) {
            ...
            binding.statusText.text = "-- уд/мин"
    ...
    private fun updateRealtimeHeartRate() {
        ...
        runOnUiThread {
            ...
            binding.statusText.text = "$displayHeartRate уд/мин"
        }
    }
    ...
    private fun calculateHeartRate() {
        if (redChannelValues.size < MOVING_WINDOW_SIZE * 2) {
            runOnUiThread {
                binding.statusText.text = "Недостаточно данных"
        ...
        runOnUiThread {
            if (heartRate > 0) {
                ...
                binding.statusText.text = "$heartRate уд/мин"
            } else {
                // Используем последнее валидное значение, если оно есть
                binding.statusText.text = "$lastValidHeartRate уд/мин (примерно)"
    ...
    private fun updateManualHeartRate() {
        ...
        runOnUiThread {
            if (heartRate > 0) binding.statusText.text = "$heartRate уд/мин"
        }
    }
